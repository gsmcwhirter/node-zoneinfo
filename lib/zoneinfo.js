...
var TZDate = module.exports = function (args){
    ...
    this.format = function (fmt_string){
        ...
        arr.forEach(function (chr, ind){
            ...
            if (Object.keys(self._formats).indexOf(chr) > -1)
            {
...
TZDate.listZonefiles = function (area, force){
    area = area || "";
    
    if (TZDate._zoneinfo_listcache[area || "_"] && !force){
        return TZDate._zoneinfo_list_cache[area || "_"];
    }
    
    var fs = require('fs');
    
    var path = TZDate._zoneinfo_path + area;
